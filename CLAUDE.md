...
`ctx` is a user-scoped CLI that stores all data in `~/.ctx/active/` and `~/.ctx/archive/`. Each topic is a folder containing one `context.md` with YAML frontmatter. The store is created automatically on first use; there is no `init` command.

### Data flow

Every command calls `cmd/helpers.go:openStore()` → `store.Open()` (resolves `~/.ctx/`, auto-creates it) → returns a `*store.Store`. Commands then call store methods which read/write `context.md` files via the `internal/frontmatter` package.
...
- **`internal/store`** — all filesystem operations: `Open`, `Create`, `All`, `Resolve`, `Save`, `Reload`, `Archive`, `Restore`, `Delete`, `Search`. `Init` is kept for test use only. Source of truth for topic status is filesystem location (`active/` vs `archive/`), not the frontmatter `status` field — `Save` auto-corrects the field. `isSafePathComponent` guards all slug-based path construction against traversal.
